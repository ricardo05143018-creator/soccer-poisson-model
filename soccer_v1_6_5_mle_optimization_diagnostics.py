"""
Poisson soccer model v1.6.5
Strict 90-min data. Added exponential time-decay (half-life = 40).
Rolling update for the final match (Spain vs Argentina).
Date: July 2026
"""
...
historical_matches = [
    ...
    {"home": "Argentina", "away": "England", "h_goals": 1, "a_goals": 1, "result": "D"},
    {"home": "France", "away": "England", "h_goals": 3, "a_goals": 5, "result": "A"}
]

# calculate time-decay weights
HALF_LIFE_MATCHES = 40.0  # 40 matches = half weight
XI = math.log(2) / HALF_LIFE_MATCHES

N_MATCHES = len(historical_matches)
for i, m in enumerate(historical_matches):
    time_diff = N_MATCHES - 1 - i
    m["weight"] = math.exp(-XI * time_diff)

# weighted baseline expected goals
total_weighted_goals = sum((m["h_goals"] + m["a_goals"]) * m["weight"] for m in historical_matches)
total_weight = sum(m["weight"] for m in historical_matches)
GLOBAL_BASELINE_LAMBDA = total_weighted_goals / (2.0 * total_weight)
...
# iterative scaling with damping and time-decay
    ...
    for i, team in enumerate(team_names):
        ...
        for m in historical_matches:
            ...
            if m["home"] == team:
                ...
                gf += m["h_goals"] * w
                ga += m["a_goals"] * w
                exp_gf_denom += betas[opp_idx] * GLOBAL_BASELINE_LAMBDA * w
                exp_ga_denom += alphas[opp_idx] * GLOBAL_BASELINE_LAMBDA * w
            elif m["away"] == team:
...
def outcome_probs(mat):
    ...
    for i in range(mat.shape[0]):
        for j in range(mat.shape[1]):
            if i > j: home += mat[i, j]
            elif i < j: away += mat[i, j]
            else: draw += mat[i, j]
    return home, away, draw


if __name__ == "__main__":
    print(f"[BOOT] Loaded v1.6.5 | Exponential Time-Decay active")
    ...
    if res.success:
        ...
        for home_team, away_team in matches_to_infer:
            lh = GLOBAL_BASELINE_LAMBDA * teams_data[home_team]["attack"] * teams_data[away_team]["defense"]
            la = GLOBAL_BASELINE_LAMBDA * teams_data[away_team]["attack"] * teams_data[home_team]["defense"]

            mat = score_matrix(lh, la, opt_rho)
            p_home, p_away, p_draw = outcome_probs(mat)
            max_idx = np.unravel_index(np.argmax(mat), mat.shape)

            print(f"\n--- INFERENCE: {home_team} vs {away_team} ---")
            print(f"  probabilities: {home_team} {p_home * 100:.2f}% | {away_team} {p_away * 100:.2f}% | Draw {p_draw * 100:.2f}%")
