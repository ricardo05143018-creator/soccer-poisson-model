"""
Poisson soccer model v1.6.0
Strict 90-min data. Dropped manual ratings for iterative scaling.
Date: July 2026
"""
...
# basic iterative scaling for attack/defense (fixed 25 rounds for now)
...
for _ in range(25):
    ...
    alphas = new_alphas / np.mean(new_alphas)
    betas = new_betas / np.mean(new_betas)
...
for m in historical_matches:
    idx_h, idx_a = team_to_idx[m["home"]], team_to_idx[m["away"]]
...
if __name__ == "__main__":
    print(f"[BOOT] Loaded v1.6.0 | Pure 90-min dataset")
    print(f"[BOOT] Max safe rho calculated as: {SAFE_RHO_UPPER:.4f}")
    ...
    if res.success:
        ...
        print(f"  optimal rho: {opt_rho:.4f} " + (
            "(WALL_TRAPPED)" if abs(opt_rho - SAFE_RHO_UPPER) < 1e-4 else "(INTERIOR)"))
